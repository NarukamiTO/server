/*
 * Araumi TO - a server software reimplementation for a certain browser tank game.
 * Copyright (c) 2025  Daniil Pryima
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

package jp.assasans.narukami.server.battleselect

import jp.assasans.narukami.server.core.IGameObject
import jp.assasans.narukami.server.core.IModelConstructor
import jp.assasans.narukami.server.net.command.IProtocolEnum
import jp.assasans.narukami.server.net.command.ProtocolEnum
import jp.assasans.narukami.server.net.command.ProtocolModel
import jp.assasans.narukami.server.net.command.ProtocolStruct

@ProtocolModel(6282101613166902704)
data class BattleParamInfoModelCC(
  val map: IGameObject,
  val params: BattleCreateParameters,
) : IModelConstructor

@ProtocolStruct
data class BattleCreateParameters(
  val autoBalance: Boolean,
  val battleMode: BattleMode,
  val clanBattle: Boolean,
  val dependentCooldownEnabled: Boolean,
  val equipmentConstraintsMode: String?,
  val friendlyFire: Boolean,
  val goldBoxesEnabled: Boolean,
  val limits: BattleLimits,
  val mapId: Long,
  val maxPeopleCount: Byte,
  val name: String?,
  val parkourMode: Boolean,
  val privateBattle: Boolean,
  val proBattle: Boolean,
  val rankRange: Range,
  val reArmorEnabled: Boolean,
  val theme: MapTheme,
  val ultimatesEnabled: Boolean,
  val uniqueUsersBattle: Boolean,
  val withoutBonuses: Boolean,
  val withoutDevices: Boolean,
  val withoutDrones: Boolean,
  val withoutSupplies: Boolean,
  val withoutUpgrades: Boolean,
)

@ProtocolEnum
enum class BattleMode(override val value: Int) : IProtocolEnum<Int> {
  DM(0),
  TDM(1),
  CTF(2),
  CP(3),
  AS(4),
  RUGBY(5),
  SUR(6),
  JGR(7),
}

@ProtocolStruct
data class BattleLimits(
  val scoreLimit: Int,
  val timeLimitInSec: Int,
)

@ProtocolStruct
data class Range(
  val min: Int,
  val max: Int,
)

@ProtocolEnum
enum class MapTheme(override val value: Int) : IProtocolEnum<Int> {
  SUMMER(0),
  WINTER(1),
  DAY(2),
  NIGHT(3),
  SUMMER_DAY(4),
  SUMMER_EVENING(5),
  SUMMER_NIGHT(6),
  WINTER_DAY(7),
  WINTER_EVENING(8),
  WINTER_NIGHT(9),
  SPACE(10),
}
